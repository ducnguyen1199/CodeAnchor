...
codeanchor/
├── bin/
│   └── anchor.js                 # CLI entry (#!/usr/bin/env node)
├── src/
│   ├── cli/
│   │   ├── commands/
...
│   │   ├── tech-detector.ts     # Detect from package.json
...
export async function initCommand() {
  ...
  const structure = await inquirer.select({
    ...
    choices: [
      { value: 'atomic', name: 'Atomic Design (atoms → molecules → organisms)' },
